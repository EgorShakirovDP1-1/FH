<template>
    <Layout>
        <div v-if="$page.props.isAdmin" id="create" class="text-center mb-0" role="region" aria-label="Admin Actions">
            <a :href="route('films.create')" class="btn btn-warning text-primary pb-2 align-items-center">
                <div class="d-flex">
                        ...
                        <div class="container text-center">
                            <div class="row justify-content-between">
                                ...
                                <div class="card col-lg-3 col-md-5 p-0 m-3 bg-primary text-light border-primary"
                                    ...
                                    <img :src="film.filmImageURL" class="card-img-top img-fluid" alt="Film Image" />
                                    <div class="card-body">
                                        <h3 class="card-title">
                                            {{ film.author }} {{ film.model }}
                                        </h3>
                                        <p class="text-white mb-0">
                                            Price Per Day: {{ film.ndc }}€
                                        </p>
                                        <p class="text-white">
                                            Price Per Km: {{ film.price_per_watch }}€
                                        </p>
    ...
    methods: {
        ...
        handleSort(value) {
            if (value === 'cheapToExpensiveDay') {
                this.films.sort((a, b) => a.ndc - b.ndc);
            } else if (value === 'expensiveToCheapDay') {
                this.films.sort((a, b) => b.ndc - a.ndc);
    ...
    computed: {
        filteredFilms() {
            ...
            if (this.pricePerKmFilter !== '') {
                films = films.filter(film => film.price_per_watch <= this.pricePerKmFilter);
            }

            if (this.pricePerDayFilter !== '') {
                films = films.filter(film => film.ndc <= this.pricePerDayFilter);
            }
            ...
            if (this.bodyTypeFilters.length > 0) {
                films = films.filter(film => this.bodyTypeFilters.includes(film.main_hero));
            }
            ...
            if (this.searchFilter !== '') {
                films = films.filter(film => film.author.toLowerCase().includes(this.searchFilter.toLowerCase()) ||
                    ...
                    film.main_hero.toLowerCase().includes(this.searchFilter.toLowerCase()) ||
